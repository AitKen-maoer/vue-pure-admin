...
export const usePermissionStore = defineStore({
  ...
  actions: {
    ...
    async changeSetting() {
      let name = storageSession.getItem("token")?.username;
      await getAsyncRoutes({ role: name }).then(res => {
        if (res.data.jsonStr && settings.isLocalRoute) {
          this.wholeRoutes = router.options.routes.filter(
            (v) => v.meta.showLink
          );
          return;
        }
        const rout = JSON.parse(res.data.jsonStr)
        addAsyncRoutes([rout]).forEach((v: any) => {
          // 防止重复添加路由
          if (
